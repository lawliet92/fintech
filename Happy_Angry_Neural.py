...
model.load_weights('/Volumes/JasonWork/Data/fer2013/Happy_Angry_Neural-22-0.85.hdf5')
...
index = np.where((label == 0.) | (label == 3.) | (label == 6.))[0].tolist()

my_data = my_data[index]
label = label[index]

index0 = np.where(label == 3.)[0].tolist()
label[index0] = 1.

index1 = np.where(label == 6.)[0].tolist()
label[index1] = 2.
...
filepath='/home/jiay/fintech/results/Happy_Angry_Neural-{epoch:02d}-{val_acc:.2f}.hdf5'
